...
-- Define the constants early in the module
dummyAccountId :: UUID
...
-- Define addEntry function before it's used
addEntry :: Discrete USD -> LedgerEntry -> Discrete USD
addEntry acc entry =
  let
    entryData = unwrap entry
    amount = toDiscrete entryData.amount
  in
    if entryData.isDebit then acc + amount
    else acc - amount
...
validateLedgerBalance entries =
  if null entries then
    ...
    in
      if sumEntries == Discrete 0 then Right unit
...
getAccountBalance entries accountId maybeUntil =
  let
    accountEntries = filter (\entry -> (unwrap entry).accountId == accountId)
      entries

    timeFilteredEntries = case maybeUntil of
      Nothing -> accountEntries
      Just until -> filter (\entry -> (unwrap entry).timestamp <= until)
        accountEntries
...
createCashDrawerAdjustment regId empId amt isAddition reason approver = do
  ...
  case validateLedgerBalance entries of
    ...
    Right _ -> do
      liftEffect $ log
        "Created balanced ledger entries for cash drawer adjustment"
      pure $ Right entries
...
generateLedgerReport
  ...
  -> Aff
       ( Array
           { account :: Account
           , openingBalance :: Discrete USD
           , activity :: Discrete USD
           , closingBalance :: Discrete USD
           }
       )
...
recordLedgerTransaction tx entries = do
  case validateLedgerBalance entries of
    Left err -> pure $ Left err
    Right _ -> do
      liftEffect $ log $ "Recording " <> show (Array.length entries)
