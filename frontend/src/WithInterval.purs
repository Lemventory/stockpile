-- This is the old, refined LiveView Module that used the interval function from FRP.Event.Time
-- I need to find a way to get this working with Routing like MenLiveView does right now
module WithInterval
  ( runLiveView
  ) where
...
import Types (Inventory(..), InventoryResponse(..), ItemCategory, MenuItem(..), QueryMode(..), Species, StrainLineage(..))
...
renderItem (MenuItem item) =
  ...
  in
    D.div
      [ klass_
          ( "inventory-item-card " <> generateClassName
