...
import Data.Finance.Money (Dense(..), Discrete(..), fromDiscrete)
import Data.Finance.Money.Extended (DiscreteMoney, fromDense', fromDiscrete', toDiscrete)
...
import Utils.Money (formatMoney, formatMoney')
                          ...
                          , DL.click_ \_ -> do
                              ...
                              case (readFloat itemQuantity) of
                                ...
                                Just qty -> void $ launchAff do
                                  ...
                                  let
                                    -- Create the Discrete first
                                    priceInCents = Int.floor (item.price * 100.0)
                                    price = Discrete priceInCents

                                    -- Convert to Discrete for our internal representation
                                    priceAsDiscrete = fromDiscrete' price

                                    -- Calculate subtotal directly as Discrete
                                    qtyAsInt = Int.floor qty
                                    itemSubtotal = fromDiscrete' (price * (Discrete qtyAsInt))

                                    -- Calculate tax as 10% of subtotal (still Discrete)
                                    taxRate = 0.1
                                    taxRateInt = Int.floor (taxRate * 100.0)
                                    itemTaxTotal = fromDiscrete' ((toDiscrete itemSubtotal * (Discrete taxRateInt)) / (Discrete 100))
                ...
                case (readFloat paymentAmountValue) of
                  ...
                  Just amount -> do
                    ...
                    let
                      ...
                      change =
                        if paymentTendered > paymentAmount then paymentTendered
                          - paymentAmount
                        else Discrete 0
    ...
    , D.div
        ...
        , D.div
            ...
            , D.div [ DA.klass_ "text-right" ]
                [ subtotalValue <#~> \amount -> text_ (formatMoney' amount) ]
            , D.div [ DA.klass_ "text-right font-semibold" ]
                [ text_ "Discount:" ]
            , D.div [ DA.klass_ "text-right" ]
                [ discountTotalValue <#~> \amount -> text_ (formatMoney' amount)
                ]
            , D.div [ DA.klass_ "text-right font-semibold" ] [ text_ "Tax:" ]
            , D.div [ DA.klass_ "text-right" ]
                [ taxTotalValue <#~> \amount -> text_ (formatMoney' amount) ]
            , D.div
                [ DA.klass_ "text-right font-semibold text-lg border-t pt-1" ]
                [ text_ "Total:" ]
            , D.div [ DA.klass_ "text-right text-lg border-t pt-1" ]
                [ totalValue <#~> \amount -> text_ (formatMoney' amount) ]
            ...
            , D.div [ DA.klass_ "text-right pt-4" ]
                [ paymentsValue <#~> \payments ->
                    let
                      paymentTotal = foldl
                        (\acc (PaymentTransaction p) -> acc + p.amount)
                        (Discrete 0)
                        payments
                    in
                      text_ (formatMoney' paymentTotal)
                ]
            , D.div [ DA.klass_ "text-right font-semibold" ]
                [ text_ "Remaining:" ]
            , D.div [ DA.klass_ "text-right" ]
